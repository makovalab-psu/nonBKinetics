...
getPvalue <- function(grp.1, grp.2, file, i, ITER, paired) {
  print(mean(grp.1))
  print(mean(grp.2))
    ...
    if (sum(success) == 0) {
      ...
      if ((uniq1 < 2) & (uniq2 < 2)) {
        ...
        if (test == "quantiles") {
          ...
          difference <- sum(T2[, 2] - T2[, 1]) / nrow(T2)
          difference_perc <- difference / mean(T2[, 1])
          T2 <- sum((T2[, 2] - T2[, 1]) ^ 2)
        }
      } else {
        B2 <- prop.test(pointmass, obs)$statistic
  ...
  examples <- unlist(lapply(1:max.iter, function(x) {
    ...
    temp = TwoPart(data, groups_perm, test = "mean")$statistic  ###HERE!!!### "quantiles" or "mean"
    return(temp)
  }))
  
  #test.diff <- Reduce(cbind,by(data, groups, quantile, probs=c(.05, .25, .5, .75, .95),na.rm=TRUE))
  #test.diff=sum((test.diff[,2]-test.diff[,1])^2)
  two.part = TwoPart(data, groups, test = "mean")   ###HERE!!!### "quantiles" or "mean"
...
run <- function(directory, filenames, paired, ITER) {
  print(paste(directory, filenames, paired, ITER))
  ...
  for (i in c("TOTAL", "MISMATCHES", "INSERTION", "DELETION")) {
    ...
    for (file in filenames) {
      ...
      if (paired == TRUE) {
        ...
        result = getPvalue(values[filled], valuesEmpty[filled], file, i, ITER, paired) #should be TRUE for paired test when using features of variable length
        print(result)
        print(
          paste(
  ...
  #OUTPUT TABLES
  print("PRINT TABLES-------------")
  names <- c("APhasedRepeatsFeatureOnly.mf", "DirectRepeatsFeatureOnly.mf", "InvertedRepeatsFeatureOnly.mf", "MirrorRepeatsFeatureOnly.mf", "ZDNAMotifsFeatureOnly.mf", "GQuadPlusFeatureOnly.mf", "GQuadMinusFeatureOnly.mf")
...
directories <-
  c(
    "/Users/alice/Desktop/projects/kinetics/errors_will/old_June30/june1/features/illumina",
    "/Users/alice/Desktop/projects/kinetics/errors_will/old_June30/june1/features/pacbio",
    "/Users/alice/Desktop/projects/kinetics/errors_will/old_June30/june1/features/diversity",
    "/Users/alice/Desktop/projects/kinetics/errors_will/old_June30/june1/features/divergence",
    "/Users/alice/Desktop/projects/kinetics/errors_will/old_June30/june1/features/TCGA"
  )
...
for (directory in directories) {
  ...
  run(directory, filenames, paired, ITER)
  #traceback()
}
