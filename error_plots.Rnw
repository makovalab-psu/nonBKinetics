...
getPvalue <- function(grp.1, grp.2, file, i, ITER, paired, parametric=FALSE) {
  ...
  TwoPart <- function(data,
    ...
    if (sum(success) == 0) {
      ...
      difference_perc = NA
      if(paired){
        B2 <- mcnemar.test(table(Group1 == point.mass,Group0 == point.mass))$statistic
      }else{
  ...
  if(!parametric){
    ...
    examples <- unlist(lapply(1:max.iter, function(x) {
      ...
      temp = TwoPart(data, groups_perm, test = "t.test", paired)$statistic  
      return(temp)
    }))
  }
  ...
  two.part = TwoPart(data, groups, test = "t.test",paired)   
...
run <- function(directory, filenames, paired, ITER, parametric=FALSE) {
  print(paste(directory, filenames, paired, ITER, parametric))
...
for (directory in directories) {
  ...
  run(directory, filenames, paired, ITER, parametric)
  #traceback()
}
