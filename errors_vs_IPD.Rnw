...
runRegression <-
  function(vectorErrors,
    ...
    mtext(
      paste("p-val:", pvalue, "slope:", slope, "r-sq", rsquare),
    ...
    myCor<-cor.test(vectorErrors,vectorIPDs,method="spearman")
    mtext(
      paste("p-value:", round(myCor$p.value,3), "cor:", round(myCor$estimate,3)),
      col = ifelse(myCor$p.value < 0.05, 'red', 'blue'),
      side = 3,
      line = 0.5,
      cex = 0.75
    )
...
#CHANGE PATH TO THE APPROPRIATE FOLDER HERE
setwd("/Users/alice/Desktop/projects/kinetics/errors_vs_IPD/october19")
