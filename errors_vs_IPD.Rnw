...
loadData <- function(fileErrors, fileIPDs) {
  ...
  return(list(
    ...
    mismatchesERRORSquantile = quantile(mergedData[["MISMATCHES"]],na.rm=TRUE),
    insertionERRORS = mean(mergedData[["INSERTION"]],na.rm=TRUE),
    insertionERRORSquantile = quantile(mergedData[["INSERTION"]],na.rm=TRUE),
    deletionERRORS = mean(mergedData[["DELETION"]],na.rm=TRUE),
    deletionERRORSquantile = quantile(mergedData[["DELETION"]],na.rm=TRUE)
  ))
}
