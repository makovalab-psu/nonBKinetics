#!/usr/bin/env python
"""
"Dilate" genomic intervals (in the sense of Minkowski/morphology set operations).

see en.wikipedia.org/wiki/Dilation_(morphology)
"""
...
def usage(s=None):
	message = """
usage: cat intervals_file | dilate_intervals <length> [options] > intervals_file
  <length>       the number of bases to add to each side of an interval
  --origin=one   intervals are origin-one, closed
  --origin=zero  intervals are origin-zero, half-open
                 (this is the default)

  Note that it is possible that the resulting intervals might extend beyond the
  end of a chromosome."""
...
def main():
	...
	for arg in argv[1:]:
		...
		elif (dilationLength == None):
			dilationLength = int_with_unit(arg)
			if (dilationLength < 0): usage("length must be non-negative")
		else:
			usage("unrecognized option: %s" % arg)

	if (dilationLength == None):
